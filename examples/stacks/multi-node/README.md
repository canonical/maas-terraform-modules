...
This example will spin up a multi-node stack that runs the `juju-bootstrap`, `maas-deploy` and `maas-config` modules. If running locally, you may require approximately 27GB of RAM to run successfully with the pre-populated constraints in `terragrunt.stack.hcl` (determined by summing the machine count and the machine constraints). It is a good example of how to use these stacks for more complex deployments.

The stack bootstraps a Juju controller, deploys 3 units of the maas-region charm (region+rack mode), 3 units of the postgresql charm, and 3 units of the HAProxy charm (with Keepalived subordinate). MAAS is then configured with example resources.
